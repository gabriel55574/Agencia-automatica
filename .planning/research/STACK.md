...
**Overall Confidence:** MEDIUM (Next.js version verified via official blog; remaining versions based on training data -- verify at install time)
...
**Why XState over simpler alternatives:**
...
Next.js Route Handler
  -> spawn('claude', ['--print', '--system-prompt', squadPrompt, '--output-format', 'json', clientContext])
  -> Stream stdout to Supabase (job progress)
  -> Parse final JSON output
  -> Store deliverables in Supabase Storage
  -> Update client pipeline state via XState
```

**IMPORTANT:** Claude Code CLI runs as a local process. For production, this means the Next.js server must run on a machine with Claude CLI installed and authenticated. This is NOT a serverless-compatible architecture for the CLI trigger routes. Use a persistent server (VPS, dedicated instance) or split: Vercel for frontend + a small backend service for CLI execution.
...
**Why BullMQ over simpler approaches:**
...
**Alternative considered: Supabase pg_cron + pg_net** -- Simpler but cannot manage long-running local CLI processes. Only works for HTTP-triggerable tasks.
...
The Claude Code CLI integration requires a persistent server process. You CANNOT deploy the entire app to Vercel/serverless and expect CLI triggers to work.
...
**Recommendation:** Start with Option A (single VPS via Railway or Render). Split later only if you need Vercel's edge network for global performance (unlikely for a solo-operator tool).

### 2. Database Schema Strategy

Use Supabase migrations (`supabase db diff` and `supabase migration new`) from day one. Never manually edit production schema. The pipeline state machine transitions map directly to database state:

```
clients -> client_phases -> phase_processes -> quality_gates -> squad_executions -> deliverables
```

### 3. XState Persistence Pattern

Client pipeline state machines must survive server restarts, deployments, and crashes. Store XState snapshots in Supabase:

```typescript
// On every state transition:
const snapshot = actor.getPersistedSnapshot();
await supabase.from('client_pipelines').update({
  machine_state: snapshot,
  current_phase: snapshot.value,
  updated_at: new Date()
}).eq('client_id', clientId);

// On server startup / page load:
const { data } = await supabase.from('client_pipelines').select('machine_state').eq('client_id', clientId).single();
const actor = createActor(pipelineMachine, { snapshot: data.machine_state });
actor.start();
```
...
npx create-next-app@latest agency-os --typescript --tailwind --eslint --app --turbopack

# Core dependencies
npm install @supabase/supabase-js @supabase/ssr xstate @xstate/react zod react-hook-form @hookform/resolvers

# UI
npx shadcn@latest init
npx shadcn@latest add button card dialog form input table tabs badge
...
# Dev dependencies
npm install -D vitest @testing-library/react @testing-library/jest-dom playwright supabase
```
...
CLAUDE_CLI_PATH=/usr/local/bin/claude  # Or wherever claude is installed
CLAUDE_CLI_MAX_CONCURRENCY=3           # Max simultaneous squad sessions
```
...
**Action item for development start:** Run `npm info [package] version` for each MEDIUM confidence package before beginning development to get exact current versions.
